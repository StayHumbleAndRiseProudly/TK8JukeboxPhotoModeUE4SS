...
1.5. If you can fully understand how to install it from the official documentation [here](https://docs.ue4ss.com/dev/guides/creating-a-lua-mod.html), the rest should be easy for you, if not, follow the next steps
(Just what I did to make it work.)
...
5. Now there is a txt file named `mods.txt` at "...\Polaris\Binaries\Win64\Mods". \
Open it to edit, and under the mods lines, write : `mod_name : 1`, should look like this (in my case with `MusicMenuHUDToggle`):
...
7. Done! Now execute the game, enter the JukeBox, select stage and track, press F9 and enjoy! (See **Usage** section to "clarify".) I added some footage, to see the actual differences.
...
Just enter the JukeBox, select stage (doesn't matter if it is "normal" or "climax"), select a track from any game, press "i" (or whatever with gamepad) to play the music (or not, do as you like), and press "F9", **enjoy I guess**.

## My Note
I'm not any specialist in this field of programming and modding, just know a few things, but as I was playing Tekken 8, I realized that I wanted that "jukebox background" with my main, (I dunno, looks fire for me) and decided to build the "tool", because it exists one for the main menu by the game developers, but not "here", and *voilà*. \
\
This is my first repository and anything positive is welcomed. For troubleshooting I think using the "Issues" tab is good, if any. (I "test" it a few times and besides the helicopter/chopper sounds my PC is making, I believe everything is fine.)
